        ...
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th v-if="visualizar.visivel || actualizar.visivel || remover.visivel"></th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="obj, chave in dadosFiltrados" :key="chave">
                    <td v-for="valor, chaveValor in obj" :key="chaveValor">
                        ...
                        <span v-if="titulos[chaveValor].tipo == 'fk'">{{ getFkNome(valor) }}</span>
                        <span v-if="titulos[chaveValor].tipo == 'data'">{{ $filters.formataDataTempo(valor) }}</span>
                        <span v-if="titulos[chaveValor].tipo == 'imagem'">
                            <img :src="'/storage/'+valor" width="30" height="30">
                        </span>  
                    </td>
                    <td v-if="visualizar.visivel || actualizar.visivel || remover.visivel">
                        ...
                        <button v-if="actualizar.visivel" class="btn btn-outline-primary btn-sm" :data-bs-toggle="actualizar.dataToggle" :data-bs-target="actualizar.dataTarget" @click="setStore(obj)">Actualizar</button>
